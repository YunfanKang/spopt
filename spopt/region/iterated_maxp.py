"""Iterated greedy Max-p regions algorithm.

The greedy construction phase is modified to adopt the iterated greedy framework.

Adapted from: Wei, Ran, Sergio J. Rey, and Elijah Knaap (2020) "Efficient
...
def checkResult(
    ...
    '''Checks if the partition is consistant
    
    Parameters
    ----------
    labels: array, required
        The array of region labels for the areas

    regionAreaDic: dictionary, required
        The dictionary with the region as the key and the list of areas in the region as the value
    
    regionAttrDic: dictionary, required
        The dictionary with the region as the key and the spatial extensive attribute value of the region
        as the value
    ...
    0 if normal, -1 if the area label does not mach the region area list, -2 if the
    region attribute value does not equal to the sum of the areal attribute value
...
def get_one_hop_neighbor_regions(
    ...
    '''Return the id of the one-hop neighbor regions of the given region IDs'''
...
def get_deconstruct_areas(
    ...
    '''Get the areas to be deconstructed by the list of region IDs'''
    deconstruct_areas = []
    for i in range(len(labels)):
        if labels[i] in regionIds:
